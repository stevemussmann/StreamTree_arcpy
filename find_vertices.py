...
class FindVertices():
	...
	def __init__(self,points,streams):
		# set workspace
		...
		splits = self.split_line(points,dissolved,"split_streams.shp") #split all lines at the sampling sites
		endpoints = self.end_points(splits,"end_points.shp") #make endpoints for resulting split streams from previous command
		...
		self.vertices = self.merge_points(erased,points,"all_vertices.shp")
	...
	def merge_points(self,erased,points,out):
